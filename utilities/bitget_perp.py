...
class PerpBitget:
    ...
    async def place_trailing_stop(self, pair: str, type: str, side: str, size: float, params: dict) -> dict:
        """
        Place un ordre de trailing stop sur Bitget
        
        Args:
            pair: La paire de trading (ex: "BTC/USDT")
            type: Le type d'ordre ("market" ou "limit")
            side: Le côté de l'ordre ("buy" ou "sell")
            size: La taille de l'ordre
            params: Les paramètres supplémentaires (trailingTriggerPrice, trailingPercent, etc.)
            
        Returns:
            dict: La réponse de l'API Bitget
        """
        pair = self.ext_pair_to_pair(pair)
        try:
            ...
            print(f"Erreur lors du placement du trailing stop: {str(e)}")
            raise e
